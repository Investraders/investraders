...
export default function RightPanel() {
  ...
  const { data: recentMsgs = [] } = useQuery({
    ...
    queryFn: () => base44.entities.DirectMessage.filter({ recipient_id: user?.id }, '-created_date', 30),
    enabled: !!user?.id,
  ...
  const { data: sentConns = [] } = useQuery({
    ...
    queryFn: () => base44.entities.Connection.filter({ requester_id: user?.id }),
    enabled: !!user?.id,
  });
  const { data: receivedConns = [] } = useQuery({
    queryKey: ['right-conns-received', user?.id],
    queryFn: () => base44.entities.Connection.filter({ recipient_id: user?.id }),
    enabled: !!user?.id,
  });
