...
export default function UpcomingEventsCalendar() {
  ...
  const { data: circles = [] } = useQuery({
    ...
    queryFn: () => base44.entities.Circle.filter({ member_ids: { $contains: user?.id } }),
    enabled: !!user?.id,
  });
  ...
  const { data: allEvents = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const results = await Promise.all(
        circleIds.map((id) => base44.entities.CircleEvent.filter({ circle_id: id, status: 'approved' }, 'event_date', 20))
      );
