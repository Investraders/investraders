...
export default function CircleAdminDashboard({ circleId, circle }) {
  const { data: posts = [] } = useQuery({
    ...
    queryFn: () => base44.entities.Post.filter({ circle_id: circleId }, '-created_date', 100),
  });

  const { data: responses = [] } = useQuery({
    queryKey: ['admin-responses', circleId],
    queryFn: () => base44.entities.CircleResponse.filter({ circle_id: circleId }, '-created_date', 100),
  });
