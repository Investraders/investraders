...
async function analyzeAndSave({ circleId, productCategory, content, authorName }) {
  ...
  const sentiment_score = result?.score ?? 0;

  await base44.entities.ProductComment.create({
    circle_id: circleId,
...
export default function ProductSentiment({ circleId, productCategory, currentUser }) {
  ...
  const { data: comments = [], isLoading } = useQuery({
    ...
    queryFn: () => base44.entities.ProductComment.filter({ circle_id: circleId, product_category: productCategory }, '-created_date', 20),
    staleTime: 30 * 1000,
