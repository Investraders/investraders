...
export default function SaveProductButton({ userId, circleId, productCategory, brandName, priceRange }) {
  ...
  const { data: saved = [] } = useQuery({
    ...
    queryFn: () => base44.entities.SavedProduct.filter({ user_id: userId, circle_id: circleId, product_category: productCategory }),
    enabled: !!userId,
  });
  ...
  const unsaveMutation = useMutation({
    mutationFn: () => base44.entities.SavedProduct.delete(saved[0].id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['saved-products', userId, circleId, productCategory] });
